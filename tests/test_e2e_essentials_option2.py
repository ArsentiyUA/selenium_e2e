...
from pageObjects.EssentialsBundlePage import EssentialsPage
...
class TestTwo(BaseClass):
    ...
    def test_choose_product(self):
        log = self.getlogger()
        choose_essentials = ProductsPage(self.driver)
        choose_essentials.learn_more_essentials()
        get_title = self.driver.title
        log.info("Asserting Page Title")
        assert ("Essentials | Aloe Care Health: Medical Alert for Seniors, Caregivers. Fall detection options." in get_title)

    def test_essentials_product(self):
        log = self.getlogger()
        essentials = EssentialsPage(self.driver)
        essentials.essentials_order()
        get_title = self.driver.title
        log.info("Asserting Page Title")
        assert ("Essentials | Aloe Care Health: Medical Alert for Seniors, Caregivers. Fall detection options." in get_title)
