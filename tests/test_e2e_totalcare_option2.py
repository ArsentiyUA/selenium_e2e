...
from pageObjects.TotalCareBundlePage import TotalCarePage
...
class TestTwo(BaseClass):
    ...
    def test_choose_product(self):
        log = self.getlogger()
        choose_totalcare = ProductsPage(self.driver)
        choose_totalcare.learn_more_totalcare()
        get_title = self.driver.title
        log.info("Asserting Page Title")
        assert ("Total Care | Aloe Care Health: Medical Alert for Seniors, Caregivers. Fall detection options." in get_title)

    def test_totalcare_product(self):
        log = self.getlogger()
        totalcare = TotalCarePage(self.driver)
        totalcare.totalcare_order()
        get_title = self.driver.title
        log.info("Asserting Page Title")
        assert ("Total Care | Aloe Care Health: Medical Alert for Seniors, Caregivers. Fall detection options." in get_title)
